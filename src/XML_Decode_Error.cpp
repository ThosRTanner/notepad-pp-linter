...
XML_Decode_Error::XML_Decode_Error(IXMLDOMParseError &error)
{
    ...
    BSTR url;    // NOLINT(cppcoreguidelines-init-variables)
    error.get_url(&url);
    ...
    BSTR text;    // NOLINT(cppcoreguidelines-init-variables)
    ...
    long code;    // NOLINT(cppcoreguidelines-init-variables)
    error.get_errorCode(&code);

    BSTR reason;    // NOLINT(cppcoreguidelines-init-variables)
...
XML_Decode_Error &XML_Decode_Error::operator=(
    XML_Decode_Error const &
) noexcept = default;
