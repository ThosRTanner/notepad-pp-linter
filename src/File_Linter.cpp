...
File_Linter::run_linter(Settings::Command const &command)
{
    ...
    // FIXME Do this when reading argument string. Then we can pass Command
    // directly to execute
    ...
    auto const [exit_code, out, err] =
        execute(command.program, args, command.use_stdin ? &text_ : nullptr);
    return std::make_tuple(args, exit_code, out, err);
}
...
void File_Linter::setup_environment()
{
    ...
    for (auto const &variable : variables_)
    {
        ...
        auto const &command{variable.second};
        auto const [res, out, err] = execute(command.program, command.args);
        if (out.length() == 0)
        {
            if (err.length() != 0)
            {
...
std::tuple<DWORD, std::string, std::string> File_Linter::execute(
    std::filesystem::path const &prog, std::wstring args,
...
) const
{
    ...
    if (not prog.empty())
    {
        ...
        if (prog.extension() == L".bat" or prog.extension() == L".cmd")
        {
